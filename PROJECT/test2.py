...
def init_db():
    ...
    # ถ้าไม่มีสินค้า ให้เพิ่มตัวอย่าง
    c.execute("SELECT COUNT(*) as cnt FROM products")
    if c.fetchone()["cnt"] == 0:
        sample_products = [
            ("Paracetamol 500mg", "ยา", 40.0, 100, "ยาแก้ปวด/ลดไข้"),
            ("Vitamin C 500mg", "วิตามิน", 120.0, 50, "เสริมภูมิคุ้มกัน"),
            ("Sunscreen SPF50", "เวชสำอาง", 350.0, 20, "ป้องกันแสงแดด"),
            ("Face Mask 50pcs", "หน้ากากอนามัย", 150.0, 200, "หน้ากากใช้แล้วทิ้ง"),
            ("Shampoo 250ml", "บำรุงผม", 180.0, 30, "แชมพูบำรุง"),
        ]
        c.executemany("INSERT INTO products (name, category, price, stock, description) VALUES (?, ?, ?, ?, ?)",
                      sample_products)
        conn.commit()
...
class PharmaCareApp:
    ...
    def show_login_screen(self):
        ...
        frame = ctk.CTkFrame(self.main_frame, corner_radius=10, width=420, height=320)
        frame.place(relx=0.5, rely=0.5, anchor="center")

        ctk.CTkLabel(frame, text="PharmaCare +", font=("Arial", 24, "bold")).pack(pady=10)
        ...
        btn_login = ctk.CTkButton(frame, text="Log in", command=login_action)
        btn_login.pack(pady=(12, 6), ipadx=10, ipady=6)

        btn_register = ctk.CTkButton(frame, text="Register (สร้างบัญชี)", fg_color=None, command=register_action)
        btn_register.pack(pady=(0, 6))
    ...
    def show_cart(self):
        ...
        def checkout_action():
            if not self.cart:
                ...
                return
            # สร้าง QR code ที่แสดงยอดชำระ (ตัวอย่าง) และบันทึก order ลง DB
            order_id = self.create_order_and_save(total)
            # สร้าง QR code image
            qr_img = self.generate_qr_for_payment(order_id, total)
            # แสดงหน้าต่างยืนยันการชำระพร้อม QR
            self.show_qr_window(qr_img, total, order_id)
            cart_win.destroy()  # close cart window
            self.cart = {}
            self.show_home_screen()

        btn_checkout = ctk.CTkButton(cart_win, text="ชำระเงิน", fg_color="green", command=checkout_action)
        btn_checkout.pack(pady=6)

    def create_order_and_save(self, total):
        conn = get_db_connection()
        c = conn.cursor()
        created_at = datetime.datetime.now().isoformat()
        # insert order
        c.execute("INSERT INTO orders (user_id, total, created_at) VALUES (?, ?, ?)",
                  (self.current_user["id"], total, created_at))
        ...
        for pid, qty in self.cart.items():
            ...
            p = c.fetchone()
            price_each = p["price"]
            c.execute("INSERT INTO order_items (order_id, product_id, qty, price_each) VALUES (?, ?, ?, ?)",
    ...
    def generate_qr_for_payment(self, order_id, total):
        # สร้างข้อความ QR อย่างง่าย (คุณอาจปรับให้เป็นข้อมูลจริงเช่น payment link)
        payload = f"PharmaCare|order:{order_id}|amount:{total:.2f}"
        ...
        qpath = os.path.join(qdir, f"order_{order_id}.png")
        img.save(qpath)

        # บันทึก path ลง orders
        conn = get_db_connection()
        c = conn.cursor()
        c.execute("UPDATE orders SET qrcode_path = ? WHERE id = ?", (qpath, order_id))
        conn.commit()
        conn.close()
        return qpath

    def show_qr_window(self, qr_path, total, order_id):
        w = ctk.CTkToplevel(self.root)
        w.title("QR Payment / ใบเสร็จ")
        w.geometry("380x520")
        ctk.CTkLabel(w, text=f"ยอดที่ต้องชำระ: {total:.2f} บาท", font=("Arial", 16, "bold")).pack(pady=8)
        # load image
        img = Image.open(qr_path).resize((260,260))
        tkimg = ImageTk.PhotoImage(img)
        lbl = ctk.CTkLabel(w, image=tkimg, text="")
        lbl.image = tkimg
        lbl.pack(pady=6)

        ctk.CTkLabel(w, text=f"หมายเลขใบสั่งซื้อ: {order_id}").pack(pady=6)
        ctk.CTkLabel(w, text="สแกน QR เพื่อชำระ (ตัวอย่าง)").pack(pady=2)

        def done_action():
            messagebox.showinfo("ชำระเงินเสร็จสิ้น", "ระบบบันทึกการชำระและออกใบเสร็จเรียบร้อย")
            w.destroy()

        ctk.CTkButton(w, text="ยืนยันชำระเสร็จ (ตัวอย่าง)", fg_color="green", command=done_action).pack(pady=10)

    # ---------- Profile ----------
    def show_profile(self):
        win = ctk.CTkToplevel(self.root)
        win.title("ข้อมูลผู้ใช้")
        win.geometry("600x500")
        ctk.CTkLabel(win, text=f"ข้อมูลผู้ใช้: {self.current_user['username']}", font=("Arial", 16, "bold")).pack(pady=8)
        # order history
        conn = get_db_connection()
        c = conn.cursor()
        c.execute("SELECT * FROM orders WHERE user_id = ? ORDER BY created_at DESC", (self.current_user["id"],))
        orders = c.fetchall()
        conn.close()

        tb = ctk.CTkTextbox(win, width=560, height=380)
        tb.pack(padx=8, pady=6)
        if not orders:
            tb.insert("end", "ยังไม่มีประวัติการสั่งซื้อ")
        else:
            for o in orders:
                tb.insert("end", f"Order {o['id']} | วันที่: {o['created_at']} | ยอดรวม: {o['total']:.2f} บาท\n")
                # items
                conn = get_db_connection()
                c = conn.cursor()
                c.execute("""
                    SELECT oi.qty, oi.price_each, p.name FROM order_items oi
                ...
                conn.close()
                for it in items:
                    tb.insert("end", f"  - {it['name']} x{it['qty']} @ {it['price_each']:.2f}\n")
                tb.insert("end", "-"*40 + "\n")
        ...
        ctk.CTkButton(win, text="ออกจากระบบ", fg_color="red", command=logout).pack(pady=6)
